"""В этом файле написан код бота.

Бот в ответ на Фамилию и Имя студента
выдает оценки по Питону
"""
import telebot
import pandas as pd
...
def load_data(department):
    """функция загружает данные из Excel-файла.
...
@bot.message_handler(func=lambda message:
                     message.text.lower() in ['чистмат', 'совбак'])
def get_department(message):
    """Функция узнает принадлежность к группе.

    Она сохраняет значение, подгружает данные
    и затем просит Фамилию и Имя студента.
    """
...
def get_grades(message):
    """По фамилии и имени ищем студента.

    В случае ошибки бот отправляет соответсвующее сообщение.
    """
...
def handle_unexpected_message(message):
    """Функция, реагирующая на неожиданные сообщения."""
    if message.chat.id not in user_data:
